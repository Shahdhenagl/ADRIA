...
export default function Discounts() {
  ...
  const reset = () => {
    setSelected(null); setQuery(''); setValue(''); setEditing(null); setEndsAt(''); setPermanent(false);
  ...
  const startEdit = (row: ProductDiscount) => {
    ...
    setStartsAt(localInput(row.starts_at)); setEndsAt(localInput(row.ends_at)); setPermanent(!row.ends_at); setLabelText(row.label_text || 'خصم خاص'); setBarcodeFormat(row.barcode_format); window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  const save = async () => {
    ...
    setSaving(true);
    const payload = { product_id: selected.id, discount_type: discountType, original_price: Number(selected.sale_price), discounted_price: Math.round(discountedPrice * 100) / 100, starts_at: new Date(startsAt).toISOString(), ends_at: permanent ? null : new Date(endsAt).toISOString(), is_active: true, label_text: labelText.trim() || null, barcode_format: barcodeFormat };
  ...
  return <div className="p-4 md:p-8 space-y-6" dir="rtl">
    ...
    <div className="grid xl:grid-cols-[1.1fr_.9fr] gap-6">
      <section className="rounded-3xl bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800 p-5 space-y-4"><h2 className="font-black">{editing ? 'تعديل خصم' : 'إضافة خصم جديد'}</h2>
        ...
        {selected && <div className="flex items-center justify-between rounded-2xl bg-slate-50 p-3"><div><div className="font-black">{selected.name}</div><div className="text-xs text-slate-500">السعر الحالي: {fmt(selected.sale_price)} · {selected.barcode || 'بدون باركود'}</div></div><button onClick={() => setSelected(null)} className="p-2 rounded-xl hover:bg-white"><X className="w-4"/></button></div>}
