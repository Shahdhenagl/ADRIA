...
export default function Discounts() {
  ...
  const save = async () => {
    ...
    // نحفظ عدد القطع الفعلي حتى في وضع «كل الكمية»؛ otherwise كانت الطباعة
    // تعتبر null = 1 وتطبع ملصقًا واحدًا فقط لكل منتج.
    const scopedQty = quantityMode === 'all' ? Math.floor(availableQuantity) : Math.floor(Number(quantityLimit) || 0);
    if (scopedQty <= 0 || (quantityMode === 'partial' && scopedQty > availableQuantity)) return alert(`حددي كمية من 1 إلى ${availableQuantity}`);
  ...
  const discountCount = (row: ProductDiscount) => {
    if (Number(row.quantity_limit) > 0) return Math.floor(Number(row.quantity_limit));
    const product = row.product || products.find(p => p.id === row.product_id);
    return Math.max(1, Math.floor(Number((product as any)?.stock_quantity ?? (product as any)?.display_quantity ?? 1)));
  };
  const print = (row: ProductDiscount) => {
    ...
    printBarcodeLabels({ name: product?.name || row.product_id, code: String(code), price: Number(row.original_price), discountPrice: Number(row.discounted_price), labelText: row.label_text || (row.barcode_format === 'clearance' ? 'تصفية' : row.barcode_format === 'sale' ? 'عرض خاص' : ''), currency: 'ج.م', count: discountCount(row) });
  };
  const printSelected = () => {
    ...
    printBarcodeLabelsBatch(selectedRows.map(r => { const p = r.product || products.find(x => x.id === r.product_id); return { name: p?.name || r.product_id, code: String(p?.barcode || p?.id || r.product_id), price: Number(r.original_price), discountPrice: Number(r.discounted_price), labelText: r.label_text || '', count: discountCount(r) }; }), { currency: 'ج.م' });
  };
