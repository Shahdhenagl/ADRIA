...
export function computeShopAvailable(rows: ShopTreasuryRows, settings: any): Bucket {
  ...
  const scopedPurchases = purchases.filter(isAfterLatestShopClosing);
  const scopedSalaries = salaries.filter(isAfterLatestShopClosing);
  const add = (sign: number, rec: any, field: string) => applySplit(net, rec, field, { sign });

  scopedOrders.filter((o: any) => !o.is_deleted).forEach((o: any) => {
  ...
  scopedOrders.filter((o: any) => !o.is_deleted && Number(o.held_deposit_amount) > 0).forEach((o: any) => {
    const split = o.held_deposit_split || o;
    addReservationSplit(heldDepositByMethod, split, 'held_deposit_amount');
  });
  scopedExpenses.filter((e: any) => isReservationReclassification(e.category) && Number(e.amount) > 0).forEach((e: any) => {
  ...
  scopedPurchases.filter((p: any) => !isMainTreasuryPurchase(p)).forEach((p: any) => {
  ...
  scopedSalaries.filter((s: any) => !isMainTreasuryExpense(s)).forEach((s: any) => add(-1, s, 'amount'));
